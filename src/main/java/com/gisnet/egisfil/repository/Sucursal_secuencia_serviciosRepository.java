...
public interface Sucursal_secuencia_serviciosRepository extends CouchbaseRepository<Sucursal_secuencia_servicios, String> {
    ...
    @Query("#{#n1ql.selectEntity} WHERE  `clave_servicio` = '#{[0]}' AND `fecha` = '#{[1]}' AND `id_sucursal` = '#{[2]}' AND #{#n1ql.filter} ")
    public List<Sucursal_secuencia_servicios> findByCalveServicio(String clave_servicio,Date fecha, String id_servicio);
}
